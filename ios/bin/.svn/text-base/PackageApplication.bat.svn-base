...
set FLEX_SDK=d:\iosair\bin\
set APP_NAME=shoot

:: Certificate files
set CERTIFICATE_FILE=flixel.p12
set PROVISIONING_FILE=flixel.mobileprovision

:: Set your password here or leave blank if you want to input it every time because you trust no one ;)
set APP_PASSWORD=9ria
...
call "%FLEX_SDK%\adt.bat" -package %SIGNING_OPTIONS% "..\%APP_NAME%-%IPATYPE%.ipa" application.xml "%APP_NAME%.swf" icons Default.png
if errorlevel 1 goto failed
echo. --------------------------------------------------------
echo. SUCCESS: IPA created in ("%APP_NAME%-%IPATYPE%.ipa")
echo.
goto end
